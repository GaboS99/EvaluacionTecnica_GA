...
public class SqlInjectionLesson8 implements AssignmentEndpoint {
  ...
  protected AttackResult injectableQueryConfidentiality(String name, String auth_tan) {
    ...
    String query = "SELECT * FROM employees WHERE last_name = ? AND auth_tan = ?"; // Consulta parametrizada

    try (Connection connection = dataSource.getConnection()) {
      try {
        PreparedStatement statement =
            connection.prepareStatement(query, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
        statement.setString(1, name); // Asigna el primer parámetro
        statement.setString(2, auth_tan); // Asigna el segundo parámetro
  ...
  public static void log(Connection connection, String action) {
    ...
    // Se corrige la inyección SQL aquí también
